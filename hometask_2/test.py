...
# Створюємо словник для зберігання кількості входжень кожного добутку x * (n - x), де n є сумою x і y, яка не була відмічена як недопустима.
allowable_products_after_p = {} 
for n in range(2, limit):
    if n not in not_allowed_say_p:
       for x in range(2, n // 2 + 1):
            product = x * (n - x)
            # Якщо цей добуток входить до списку розмови та має більше одного входження, додаємо його до словника.
...
# Створюємо словник для зберігання кількості входжень суми x + y, яка може бути поділена на x і (n - x), коли S говорить "Я не знаю".
allowable_say_s = {}  
for n in range(2, limit):
    if n not in not_allowed_say_p:
       for x in range(2, n // 2 + 1):
           product = x * (n - x)
           # Якщо цей добуток входить до списку розмови та має тільки одне входження, додаємо його до словника.
...
for n in allowable_say_s: 
    if allowable_say_s[n] == 1:
        for x in range(2, n // 2 + 1):
            product = x * (n - x)
            # Якщо цей добуток входить до списку розмови та має тільки одне входження, виводимо його разом з значеннями x та (n - x).
            if product in allowable_products_after_p and allowable_products_after_p[product] == 1:
